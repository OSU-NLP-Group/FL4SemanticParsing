...
class SpiderDataManager(ABC):
    # @abstractmethod
    def __init__(self, args, process_id=0, num_workers=1):
    ...
    def _load_federated_data_server(self, test_only=True, test_cut_off=None):
        ...
        for client_idx in tqdm(
                ...
                ["partition_data"].keys(),
                    desc="Loading index from h5 file."):
                train_index_list.extend(
                    partition_file["partition_data"]
                    [client_idx]["train"][()])
                local_test_index_list = partition_file[
                    "partition_data"][client_idx]["test"][()]
                test_index_list.extend(local_test_index_list)    
        ...
        test_data = self.read_instance_from_h5(data_file, test_index_list, "test", desc=" test data of server [_load_federated_data_server]")
    ...
    def _load_federated_data_local(self):
        ...
        for client_idx in self.client_index_list:
            ...
            test_index_list = partition_file["partition_data"][str(client_idx)]["dev"][()]

            train_loader = self.read_instance_from_h5(
                    data_file, train_index_list, "train", desc=" train data of client_id=%d [_load_federated_data_local] "%client_idx)
            test_loader = self.read_instance_from_h5(
    ...
    def _load_data_loader_from_cache(self, client_id):
        ...
        cached_features_file = os.path.join(
            ...
            model_args.cache_dir,  "spider_cached_" + args.partition_method + "_" + str(client_id)
        )
    ...
    def read_instance_from_h5(self, data_file, index_list, section, desc):
        train_dataset = []
        test_dataset = []
        if section == "train":
            for i in tqdm(index_list, desc="Loading data from h5 file." + desc):
                ...
                example = json.loads(data_file["train/example/" + str(i)][()])
                train_dataset.append(example)
            return train_dataset
        elif section == "test":
            for i in tqdm(index_list, desc="Loading data from h5 file." + desc):
                # example = json.loads(data_file["test/example/" + str(i)][()].decode("utf-8"))
                example = json.loads(data_file["test/example/" + str(i)][()])
