...
class Trainer:
    ...
    def __init__(
        ...
        # print(args)
        # print("args.place_model_on_device:", args.place_model_on_device)
    ...
    def train(
        ...
        **kwargs,
        ...
        # print("arg.max_steps:", args.max_steps)
        # import pdb
        # pdb.set_trace()
        ...
        print("self.fl_algorithm =", self.fl_algorithm)
        if self.fl_algorithm == "FedProx":
            import copy
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        if self.fl_algorithm == "FedProx":
                            tr_loss += self.training_step(model, inputs, global_model)
                        else:
                            tr_loss += self.training_step(model, inputs)
                else:
                    if self.fl_algorithm == "FedProx":
                        tr_loss += self.training_step(model, inputs.to(self.device), global_model)
                    else:
                        tr_loss += self.training_step(model, inputs.to(self.device))
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    log_loss = self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, ignore_keys_for_eval)
                    if log_loss != None:
                        self.train_loss_list.append(log_loss) 
                else:
        ...
        self.loss_signal = max(self.train_loss_list) - min(self.train_loss_list)
        # self.relative_loss_signal = self.loss_signal/max(self.train_loss_list)
        # self.relative_loss_signal = self.loss_signal/min(self.train_loss_list)
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval):
        ...
        if self.control.should_log:
            ...
            log_loss = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["loss"] = log_loss
    ...
    ### modify for FedProx
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], global_model = None) -> torch.Tensor:
        """
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels = self.prediction_step(model, inputs.to(self.device), prediction_loss_only, ignore_keys=ignore_keys)
