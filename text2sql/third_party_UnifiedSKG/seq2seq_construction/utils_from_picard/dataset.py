...
from seq2seq_construction.utils_from_picard.bridge_content_encoder import get_database_matches
import re
import random
...
class DataArguments:
    dataset: str = field(
        metadata={"help": "The dataset to be used. Choose between ``spider``, ``cosql``, or ``cosql+spider``, or ``spider_realistic``, or ``spider_syn``, or ``spider_dk``, or ``michigan``."},
    )
    dataset_paths: Dict[str, str] = field(
        default_factory=lambda: {
            ...
            "michigan": "./seq2seq/datasets/michigan"
        },
    ...
    metric_paths: Dict[str, str] = field(
        default_factory=lambda: {
            ...
            "michigan":"./seq2seq/metrics/michigan"
        },
    ...
    def get_column_str(table_name: str, column_name: str) -> str:
        ...
        if schema_serialization_with_db_content:
            matches = get_database_matches(
                ...
                db_path=(db_path + "/" + db_id + "/" + db_id + ".db"),
            )
            if matches:
