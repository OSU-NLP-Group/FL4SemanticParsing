...
from seq2seq.utils.michigan import michigan_add_serialized_schema, michigan_pre_process_function
...
def load_dataset(
    ...
    _michigan_dataset_dict: Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths["michigan"], cache_dir=model_args.cache_dir
    )
    _michigan_metric: Callable[[], Metric] = lambda: datasets.load.load_metric(
        path=data_args.metric_paths["michigan"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )
    _michigan_add_serialized_schema = lambda ex: michigan_add_serialized_schema(
        ex=ex,
        data_training_args=data_training_args,
    )
    _michigan_pre_process_function = lambda batch, max_source_length, max_target_length: michigan_pre_process_function(
        batch=batch,
        max_source_length=max_source_length,
        max_target_length=max_target_length,
        data_training_args=data_training_args,
    ...
    elif data_args.dataset == "michigan":
        metric = _michigan_metric()
        dataset_splits = prepare_splits(
            dataset_dict=_michigan_dataset_dict(),
            add_serialized_schema=_michigan_add_serialized_schema,
            pre_process_function=_michigan_pre_process_function,
            **_prepare_splits_kwargs,
        )
