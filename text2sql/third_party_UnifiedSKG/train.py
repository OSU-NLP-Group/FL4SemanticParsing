...
def main() -> None:
    ...
    # only use table schema, remove any table content
    import json
    table_dict_path = "/home/zhang.11535/UnifiedSKG/eight_data/table_dict.json"
    with open(table_dict_path, "r") as f:
        table_dict = json.load(f)
    # args.dataset.data_name = "advising"

    for i in range(len(seq2seq_train_dataset)):
        item = seq2seq_train_dataset[i]
        item['serialized_schema'] = table_dict['serialized_schema'][args.dataset.data_name]
        item['struct_in'] = table_dict['struct_in'][args.dataset.data_name]
    for i in range(len(seq2seq_eval_dataset)):
        item = seq2seq_eval_dataset[i]
        item['serialized_schema'] = table_dict['serialized_schema'][args.dataset.data_name]
        item['struct_in'] = table_dict['struct_in'][args.dataset.data_name]
    for i in range(len(seq2seq_test_dataset)):
        item = seq2seq_test_dataset[i]
        item['serialized_schema'] = table_dict['serialized_schema'][args.dataset.data_name]
        item['struct_in'] = table_dict['struct_in'][args.dataset.data_name]
    # import pdb
    # pdb.set_trace()
    ...
    early_stopping_callback = EarlyStoppingCallback(early_stopping_patience=args.seq2seq.patience if args.seq2seq.patience else 5) ##original paper using 5, we change it to a larger one
